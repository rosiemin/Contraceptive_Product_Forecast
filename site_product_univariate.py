...
if __name__ == "__main__":
    ...
    df_use = df[['calendar', 'site_code', 'product_code', 'stock_distributed']].copy()
    ...
    all_preds = []
    all_names = ss['product'].unique()
    all_names = ss['site'].unique()

    all_names = [s + ' ' + p for s in site_names for p in product_names]

    for p in tqdm(all_names):
        sub_df = df_use[(df_use['product_code']== p.split()[1]) & (df_use['site_code']== p.split()[0])]
        
        r = pd.date_range(start=df_use.calendar.min(), end=df_use.calendar.max(), freq = 'MS')
        sub_df = sub_df.reindex(r).fillna(0).rename_axis('calendar')
        sub_df['site_code'] = p.split()[0]
        sub_df['product_code'] = p.split()[1]
        sub_df['calendar'] = sub_df.index
        sub_df['stock_distributed'] = sub_df['stock_distributed'].copy() + 1
        ...
        if len(sub_df):
            all_preds.append(predict_SARIMA(sub_df, params, params_seasonal, p.split()[0], p.split()[1]))
        else:
            temp_df = pd.DataFrame([{'date':'2019-07-01', 'prediction': 1, 'month': 7, 'year': 2019, 'product': p.split()[1], 'site': p.split()[0]}, {'date':'2019-08-01', 'prediction': 1, 'month': 8, 'year': 2019, 'product': p.split()[1], 'site': p.split()[0]}, {'date':'2019-09-01', 'prediction': 1, 'month': 9, 'year': 2019, 'product': p.split()[1], 'site': p.split()[0]}, {'date':'2019-10-01', 'prediction': 1, 'month': 10, 'year': 2019, 'product': p.split()[1], 'site': p.split()[0]}])
